...
class Station(object):
    ...
    def get_playlist(self):
        ...
        return deque([Song(self.pandora, i) for i in playlist])
...
class Playlist():
    ...
    def getNextSong(self):
        if not self.listOfSongs:
            self.refreshListOfSongs()
            self.currentSong = self.listOfSongs.popleft()
            if not self.currentSong:
                raise PandoraError('Can\'t find next song')
        self.currentSong = self.listOfSongs.popleft()
        return self.currentSong

    def refreshListOfSongs(self):
